<?php 
    ...
    if (isset($_SESSION['unique_id'])) {
        ...
        $sql ="SELECT * FROM messages
                        RIGHT JOIN users ON users.unique_id = messages.outgoing_msg_id
        ...
        if (mysqli_num_rows($query) > 0) {
            while ($row = mysqli_fetch_assoc(($query))) {
                if ($row['outgoing_msg_id'] === $outgoing_id) { //if this tru, then this is the sender
